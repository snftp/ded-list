...
void InsertAtTheStart(list_type *list, double value) {
    InsertElement(list, value, 0);
}

void DeleteElement(list_type *list, int physical_index) {
    list->free[physical_index] = {
        .value = 0.0,
        .next_index = list->free[0].next_index,
        .previous_index = 0
    };

    (list->free[(list->free[0]).next_index]).previous_index = physical_index;
    (list->free[0]).next_index = physical_index;
    ...
    list->nodes[physical_index] = { .next_index = FREE };
...
void InsertElement(list_type *list, double value, int physical_index) {
    ...
    int new_physical_index = free;
    (list->free[0]).next_index = (list->free[free]).next_index;
    (list->free[free]).previous_index = 0;
...
void ConstructList(list_type *list) {
    list->nodes[0] = {};
    list->free[0] = { .next_index = 1 };

    for (int i = 1; i < LIST_SIZE - 1; i++) {
        list->nodes[i] = { .value = 0.0, .next_index = i + 1, .previous_index = 0 };
        list->free[i] = { .value = 0.0, .next_index = i + 1, .previous_index = 0 };
    }

    list->nodes[LIST_SIZE - 1] = { .previous_index = LIST_SIZE - 2 };
    list->free[LIST_SIZE - 1] = { .value = 0.0, .next_index = END, .previous_index = LIST_SIZE - 2 };
...
void DestructList(list_type *list) {
    for (int i = 0; i < LIST_SIZE; i++) {
        ...
        list->free[i] = { .value = NAN, .next_index = -1, .previous_index = -1};
    }
}
